...
<div class="hero-section">
        ...
        <div class="hero-overlay">
            @php
            // حساب عدد الرسائل غير المقروءة واسم المرسل
            $unreadMessages = \DB::table('messages')
                ->join('users', 'messages.sender_id', '=', 'users.id')
                ->select('users.name as sender_name', \DB::raw('count(*) as total'))
            ...
            <div class="custom-notification animate__animated animate__fadeInDown">
                @foreach ($unreadMessages as $msg)
                    <div class="notification-item">
                        ...
                        لديك {{ $msg->total }}  رسائل جديدة من <strong>
            ...
            <div class="col-lg-4 col-md-6 doctor-item" style="text-align: center">
                <div class="doctor-card">
                    <div class="doctor-image">
                        ...
                        <img src="{{ asset('storage/' . $doctor->image) }}" alt="{{ $doctor->name }}" loading="lazy"> 
                        @else 
                        <img src="User/img/p.png" alt="{{ $doctor->name }}" loading="lazy">
                        @endif
                        {{-- <img src="{{ asset('storage/' . ($doctor->image ?? 'User/img/p.png')) }}" alt="{{ $doctor->name }}" loading="lazy">  --}}
                        <div class="doctor-badge">
                            <i class="fas fa-user-md"></i>
                        </div>
                    ...
                    <div class="doctor-content" style="position: relative; padding: 1rem;">
                        <h3 style="font-weight: 700; font-size: 1.5rem; margin-bottom: 0.25rem;">{{ $doctor->name }}</h3>
                        <p class="text-muted mb-1" style="font-size: 0.95rem;">
                            <i class="fas fa-stethoscope me-1" style="color:#3498db;"></i> {{ __('web.' . $doctor->specialization) }}
                        </p>
                        <p class="small" style="font-size: 0.85rem; color: #555;">
                            <i class="fas fa-hospital me-1" style="color:#2ecc71;"></i> {{ $doctor->hospital->name ?? '' }}
                        </p>
                        ...
                        @if ($unreadCount > 0)
                            <a href="{{ route('chat.index') }}" 
                               style="position: absolute; top: 10px; right: 15px; text-decoration: none; display: flex; align-items: center;">
                        ...
                        <div class="doctor-actions mt-3">
                            <a href="{{route('chat.show',$doctor->id)}}" class="btn btn-action btn-sm">
                                
                                <i class="fas fa-eye"></i> <span>{{ __('web.ChatWithDoctor') }}</span>
                            </a>
