        ...
        <div class="appointments-content" dir="{{ app()->getLocale() == 'ar' ? 'rtl' : 'ltr' }}">
                ...
                <div class="table-container">
                    <table class="appointments-table">
                        ...
                        <tbody>
                            ...
                            <tr class="{{ $appointment->status == 'confirmed' ? 'confirmed' : ($appointment->status == 'cancelled' ? 'cancelled' : 'pending') }}">
                                <td>{{ $index + 1 }}</td>
                                <td>
                                    <div class="doctor-info">
                                        {{-- <img src="{{ asset($appointment->doctor->image ?? 'images/default-doctor.png') }}" alt="صورة الطبيب" class="doctor-image"> --}}
                                        <img src="{{ asset('storage/' . $appointment->user->image) }}" alt="{{ $appointment->user->name}}" class="doctor-image">
            ...
            @else
                <div class="empty-state">
                    ...
                    <p>ليس لديك أي مواعيد طبية مسجلة حالياً</p>
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                ...
                error: function(xhr) {
                    Swal.fire('حدث خطأ', 'حدث خطأ أثناء محاولة الإلغاء', 'error');
                }
            });
        }
    });
